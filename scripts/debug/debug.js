/**
 * Debug tools, based on hashgrid by Jon Gibbins, accessibility.co.uk
 *
...
(function ($) {
	...
	var hashgrid = function(set) {
		...
		var options = {
			...
			showScreen: 'w',          // show screen resolution
		...
		/**
		 * Build the toolset for responsive.
		 */
		function showResTools() {
	    var cssObj = {
        'position'          : 'fixed',
        'bottom'            : '40px',
        'right'             : '40px',
      ...
      var h1 = $('<h1 id="responsive-size-info" />').css(cssObj).text(w + ' x ' + h + ' px');
      ...
      $(window).resize(function(){
        ...
        $('#responsive-size-info').text(w + ' x ' + h + 'px');
      });
    }
    
    function hideResTools() {
      $('#responsive-size-info').each(function() {
        $(this).remove();
      });
    }
	...
	/**
	 * Functions to add and clear color per panel-pane type in Drupal
	 */
	var addColortype = function() {
		$('div.pane-views-panes').css('background-color', 'rgba(240,0,232,.1)');
		$('div.pane-block').css('background-color', 'rgba(0,34,240,.1)');
		$('div.block-panels-mini').css('background-color', 'rgba(124,240,0,.1)');
	}
	
	var clearColortype = function() {
		$('div.pane-views-panes').css('background-color', '');
		$('div.pane-block').css('background-color', '');
		$('div.block-panels-mini').css('background-color', '');
	}
